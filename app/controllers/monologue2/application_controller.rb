class Monologue2::ApplicationController < ApplicationController
  include Monologue2::ControllerHelpers::User

  layout Monologue2::Config.layout if Monologue2::Config.layout # TODO: find a way to test that. It was asked in issue #54 (https://github.com/jipiboily/monologue/issues/54)

  before_filter :recent_posts, :all_tags, :archive_posts

  def recent_posts
    @recent_posts = Monologue2::Post.published.limit(3)
  end

  def all_tags
    @tags = Monologue2::Tag.order("name").select{|t| t.frequency>0}
  ...
  def archive_posts
    ...
    posts = Monologue2::Post.published.limit(100)
    if posts.length > 0
