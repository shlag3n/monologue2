class Monologue2::Post < ActiveRecord::Base
  ...
  scope :default,  -> {order("published_at DESC, monologue2_posts.created_at DESC, monologue2_posts.updated_at DESC") }
  ...
  def tag!(tags_attr)
    self.tags = tags_attr.map(&:strip).reject(&:blank?).map do |tag|
      Monologue2::Tag.where(name: tag).first_or_create
    end
  end

  def full_url
    "#{Monologue2::Engine.routes.url_helpers.root_path}#{self.url}"
  end
  ...
  def self.page p
    per_page = Monologue2::Config.posts_per_page || 10
  ...
  def url_do_not_start_with_slash
    errors.add(:url, I18n.t("activerecord.errors.models.monologue2/post.attributes.url.start_with_slash")) if self.url.start_with?("/")
  end
