...
describe Monologue2::User do
  it { validate_presence_of(:name) }
  it { validate_presence_of(:email) }
  it { validate_uniqueness_of(:email) }

  context "delete user" do
    let(:user_without_post) { stub_model(Monologue2::User) }
    let(:user_with_post) do
      user = stub_model(Monologue2::User)
      user.stub(:posts => [stub_model(Monologue2::Post)])
      user
    end

    let(:user) { Monologue2::User.new }
